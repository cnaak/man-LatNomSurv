...
\title{Lattice Nomenclature Survey from LGA to Modern LBM}
\author{C.~Naaktgeboren\footnote{Corresponding author: NaaktgeborenC$\cdot$PhD\textcircled{a}gmail$\cdot$com.}}
\author{F.~N.~de Andrade}
\affiliation{Universidade Tecnológica Federal do Paraná (UTFPR), Guarapuava, Brazil. Grupo de Pesquisa em Ciências Térmicas.}
\begin{abstract}
...
\section{Lattice Nomenclature Survey}
    ...
    \subsubsection{Inception Period}
    ...
    lengths was proposed---named `face-centered rectangular'  by  the  author---whose  weights  exactly  recover  those  of  the
    well-known D2Q9 lattice for $a = b$, over which the argument that valid weights `[...] \emph{can always be found by choosing
    a large enough set of} (lattice) \emph{velocity vectors\/} [...]'~\cite{1991-KoelmanJMVA-EuroPhysLett}.
    ...
    \subsubsection{Late 1990's to mid-2000's}
    ...
    \subsubsection{Shan and Coauthors}
    ...
    \subsubsection{Philippi and Coauthors}
    ...
    %
    \begin{quote}
        ``\textit{[...]~$N$th-order  approximation  to  the  [Maxwell-Boltzmann]  equilibrium  distribution  is  required   when
        $N$th-order macroscopic equilibrium moments are to be correctly described in [lattice-Boltzmann methods,]\/}''
    \end{quote}
    %
    ...
    \subsubsection{Chikatamarla and Karlin}
    ...
    \begin{quote}
        ``\textit{[...]~discrete velocities [$\mathbf{c}_i$] in the $D$-dimensional case are tensor products of  $D$  copies  of
        the one-dimensional velocities, whereas the corresponding weights [$W_i$] are algebraic products  of  the  corresponding
    ...
    $\pm 1$,  $\pm  3$\}',  or  `zero-one-three'  1-D  lattice;  the  `D1Q5-ZOT  lattice',  defined  as  ``\textit{the  shortest
    ...
    \subsubsection{From 2010 to 2020}
...
\section{Discussion}
    ...
    types and features, as well as of the existence of any concise  \emph{naming}---not  velocity  \emph{listing}---system  that
    would allow for uniqueness by ruling out name ambiguity.
...
\section{Citations by Year}
    ...
    The following are the citations indexed by year in chronological order: %% \input{./yearReport.tex}
