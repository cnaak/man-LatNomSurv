...
\section{Discussion}
    ...
    Aiming at space-filling, regular, Bravais types in one- to three-dimensional Euclidean spaces, the authors conjecture that a
    scheme with ($\alpha$)~energy-level primitives, ($\beta$)~that allows for operations such as (tensor product) extensions and
    shiftings; is able to produce relatively \emph{concise} and  \emph{unambiguous}  lattice  names,  while  being  sufficiently
    \emph{generic} within its category. This is a work in progress planned to appear soon.
