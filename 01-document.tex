...
\section{Discussion}
    ...
    `DdQb' template of 1990 due
    to Qian~\cite{1990-QianYH-Paris}, and of 1991 of Qian and coauthors~\cite{1991-QianYH+LallemandP-AdvKinTheoContMech} seems to be the
    closest thing to a present-time
    de-facto standard for LB stencil naming, being thus acknowledged on research~\cite{2008-RubinsteinR+LuoLS-PhysRev} and on review~\cite{} papers, as well as
    textbooks~\cite{}. Nonetheless, reaching this current status hasn't been a quick process, as it seemingly took a considerable amount of years until the naming scheme became widely adopted in
    the LB literature, as evidenced by the lack of its usage in the 1998 review
    paper of Chen and coauthors, which refer to some of them as
    `LBM models based on 21 and 25
    velocities'~\cite[p.~357]{1998-ChenS+DoolenGD-AnnuRevFluidMech}.

    was not after many years after its first appearanec in 1990 that the `DdQb'
    scheme 

    Moreover, as far as naming systems
    Some naming schemes seem to have

    Aiming  at  space-filling,  regular,  Bravais  types  in  one-  to   three-dimensional   Euclidean   spaces,   the   authors
    conjecture~\cite{2020-NaaktgeborenC+AndradeFN-BravLatNam-engrXiv-rev00}   that   a   scheme   with   ($\alpha$)~energy-level
    primitives, ($\beta$)~that allows for operations such as (tensor product) extensions  and  shiftings;  is  able  to  produce
    relatively \emph{concise} and \emph{unambiguous} lattice names, while being sufficiently \emph{generic} within its category.
