...
\section{Discussion}
    ...
    over the years, with each new variety either introduced as to accomodate or reflect a new aspect brought in the research, or
    to organize and distinguish multiple lattices within a publication.

    The present survey is unaware of any published effort in the direction of major  standardizations  across  multiple  lattice
    types and features.

    Lattice naming variety may be due to (i)~the inherent decentralized  nature  of  research,  (ii)~the  inherent  novelty  and
    ...
    On this last aspect, the history of the method has seen (a)~space-filling, regular, Bravais types in square  and  triangular
    (hexagonal) 2D spaces, and cubic ones in 3D spaces; (b)~regular,  non-space-filling  ones;  (c)~irregular  ones;  and  those
    (iv)~based on spherical coordinate systems. Moreover, there are even those with shifted  reference  frame,  and  those  more
    heavily focused on spatial space discretization rather than on momentum space. This observation alone may at best  difficult
    efforts in creating concise, unambiguous lattice naming schemes of general scope.
